...
- **Wifi-presence (mDNS + cached-IP probe).** Each robot publishes itself over mDNS (Pi: avahi-daemon + `/etc/avahi/services/betterrobot.service`; ESP32: `MDNS.begin/addService` from `ESPmDNS`) and exposes `GET :81/health` returning `{ok, type, robotId, ip, uptime_s, pi_robot_service?}`. Dashboard probes each paired robot's `<name>.local:81/health` AND its last-seen BLE-reported IP in parallel; first OK wins. No internet rendezvous for robot presence — pulled by the dashboard, not pushed to a server. signal.neevs.io stays in the codebase for phone-pair (genuinely cross-network) but no longer hosts robot ads.
