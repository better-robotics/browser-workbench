...
- **Tag content:** NDEF URL → `https://better-robotics.github.io/?pair=<robot-id>`. Dashboard reads `pair` from `location.search`, filters BLE scan to that one device.
- **Android Chrome:** full flow end-to-end. Tap → URL → filtered scan → confirm.
- **iPhone:** iOS opens the URL but Web Bluetooth is unavailable. Workaround leverages the existing phone↔desktop pair layer (`signal.neevs.io`, signed pair-request, `phone.html`). Encode `phone.html?pair=<robot-id>` instead. Phone forwards `{type:"pair-robot", robotId}` over the existing WebRTC data channel; desktop surfaces a "Phone wants to pair robot-7 — click to confirm" banner. The desktop click is required because `navigator.bluetooth.requestDevice` needs a user gesture — message arrival doesn't count. Cross-network works for free (signal.neevs.io already pays for it).
- **Bootstrap caveat:** first-ever use still needs the existing phone↔desktop pair ceremony. Subsequent taps route through.
