...
- [ ] **Phone attached-mode:** mount the phone on a robot via the robot card's "Mount camera" picker. Phone screen flips to black-with-Stop-button (no operator chrome). Turning on a local cam's "Send to phone" role fills the phone screen with the laptop cam. Detaching restores normal phone UI. Disconnect mid-attach → phone shows normal reconnect surface; reconnecting re-flips to attached.
- [ ] **Pip face on attached phone (default):** mount the phone; default is `phoneAttachedMode: "pip-face"` so the screen shows two cyan rounded-rect eyes on black. Eyes blink at jittered 2–5s intervals when idle. Issuing `/demo dance` → eyes shift direction in sync with motor calls. `get_robot_detections` → eyes scan left/right oscillating. `ask_human` → eyes rotate asymmetrically (raised brows). Watcher fire (`/demo stopsign` with a stop sign held up) → eyes briefly widen (alert), then halted-squint after the halt. Stop button still halts.
