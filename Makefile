...
setup:
	...
	# NimBLE-Arduino replaces the built-in Bluedroid BLE stack. On classic
	# ESP32-CAM this returns ~58 KB of internal DRAM (heap 19→77 KB at end
	# of setup) so WiFi init gets all 4 RX DMA buffers instead of 1. Pinned
	# to 2.5.0 — 1.4.x aborts esp_bt_controller_init with ESP_ERR_INVALID_STATE
	# on arduino-esp32 3.x (IDF 5.x); 2.x is the IDF-5-native line.
...
compile: gen-uuids
	@# Stamp git SHA (or "dev" when git's unavailable) so fw-info carries
	@# a traceable commit short-hash — matches the Pi's version.py convention.
	@echo "// Generated by make; do not edit." > firmware/$(SKETCH)/version.h
	@echo "#define GIT_SHA \"$$(git rev-parse --short=7 HEAD 2>/dev/null || echo dev)\"" >> firmware/$(SKETCH)/version.h
	@# QIO flash mode bypass: arduino-esp32's esp32cam board entry has a
	@# bug where selecting "QIO" sets DIO. Forcing via build-property gives
	@# us the real QIO at 80 MHz — ~150 ms boot saved + ~10-15% XIP speedup.
	@# AI-Thinker modules ship QIO-capable flash; flip back to dio if any
	@# field unit fails to boot under QIO peak current.
	arduino-cli compile --fqbn "$(FQBN)" --build-property build.flash_mode=qio --build-path "$(BUILD_DIR)" firmware/$(SKETCH)
