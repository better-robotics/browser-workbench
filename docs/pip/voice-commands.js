...
const PATTERNS = [
  {
    name: "drive_forward",
    rx: /^(?:drive|go|move|head|come|run|walk)\s+(?:forwards?|fwd|ahead|straight)\b/i,
    build: (t) => ({ l:  SPEED, r:  SPEED, duration_ms: parseDuration(t, PULSE_MS) }),
  },
  {
    name: "drive_backward",
    rx: /^(?:drive|go|move|reverse|back\s+up)\s+(?:back(?:wards?)?|reverse)\b/i,
    build: (t) => ({ l: -SPEED, r: -SPEED, duration_ms: parseDuration(t, PULSE_MS) }),
  },
  {
    name: "turn_left",
    rx: /^(?:turn|rotate|go|veer|head)\s+(?:to\s+the\s+)?left\b/i,
    build: (t) => ({ l: -SPEED, r:  SPEED, duration_ms: parseDuration(t, TURN_MS) }),
  },
  {
    name: "turn_right",
    rx: /^(?:turn|rotate|go|veer|head)\s+(?:to\s+the\s+)?right\b/i,
    build: (t) => ({ l:  SPEED, r: -SPEED, duration_ms: parseDuration(t, TURN_MS) }),
  },
  {
    name: "spin_left",
