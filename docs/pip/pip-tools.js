...
// successful pulse (used by drive_distance_cm to accumulate cm).
// Returns { results, gatedAt }.
...
const ALL_TOOLS = [
  ...
  },
  {
    ...
    description: "Start the camera stream on a robot (synthetic click on the same Start button the user sees on the Camera card). Idempotent — returns success if already running. Required before view_robot_frame, get_robot_detections, or start_robot_watcher have a frame to act on. NOTE: if get_robot_state reports a 'phone' camera, skip this — a phone mounted as the robot's eye is already streaming and view_robot_frame works immediately.",
    input_schema: {
      type: "object",
      properties: { id: { type: "string" } },
      required: ["id"],
...
// detector switches) take effect without a page reload. Detector-backed tools
...
async function dispatch(name, input) {
  switch (name) {
    ...
    case "get_robot_state": {
      ...
      // Surface every available camera source — without this, fwInfo's
      // `webrtc: false` on a camera-less ESP32 reads as "no eyes" even
      // when a phone is mounted streaming a live feed.
      const cameras = listCameraSources(e).map(s => s.label);
      return {
        id: e.id, name: e.name, type: e.fwType ?? null,
        status: e.status,
    ...
    case "start_robot_camera": {
      ...
      // A phone mounted as the robot's eye IS the camera — nothing to
      // start. Without this short-circuit, ESP32 robots with no native
      // camera fail the start-button lookup and Pip wrongly concludes
      // "no camera capability" even though a phone feed is streaming.
      const sources = listCameraSources(e);
      const phone = sources.find(s => s.label === "phone");
      if (phone) {
        return { ok: true, running: true, source: "phone", note: "phone camera mounted on this robot — already streaming; use view_robot_frame to see it" };
      }
      ...
      if (node.querySelector(`[data-action="camera-stop"]`)) {
        return { ok: true, running: true, source: "primary", note: "camera already running" };
      }
      ...
      btn.click();
      return { ok: true, started: true, source: "primary" };
    }
