...
export function persist() {
  ...
  for (const e of state.devices.values()) {
    out.push({
      ...
      // transform to propagate). Per-robot because different chassis mount
      // their cameras differently.
      cameraFlip: !!e.cameraFlip,
    });
  }
...
export function makeEntry(id, name, fwType = null, { autoReconnect = false, lastConnectedAt = 0, arucoMarkerId = null, cameraFlip = false } = {}) {
  return {
    ...
    // helpers.js's "Attach to robot" gesture; consumed by camera-frame +
    // pip-tools so detector tools can reason over multiple cameras at once.
    // Cleared on detach or phone disconnect. attachedFromPhoneId is the
    // source phone's pairing roomId so detach can find it.
