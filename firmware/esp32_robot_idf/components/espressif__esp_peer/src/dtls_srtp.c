...
dtls_srtp_t *dtls_srtp_init(dtls_srtp_cfg_t *cfg)
{
    ...
    do {
        ...
        // PATCH: force CLIENT regardless of caller. libpeer's binary blob
        // unconditionally passes ROLE_SERVER (verified via DeepWiki source
        // index + observed captures: a=setup:passive in every emitted libpeer
        // SDP). As DTLS server, mbedTLS bails on Chrome's ~1413-byte
        // fragmented ClientHello with FEATURE_UNAVAILABLE. As CLIENT, chip
        // sends a small ClientHello and Chrome handles whatever it receives.
        // Key derivation in dtls_srtp.c does the right local↔remote swap
        // per RFC 5764. Paired with setup:passive→active rewrite in
        // webrtc_peer.c's rewrite_answer_mid().
        dtls_srtp->role = DTLS_SRTP_ROLE_CLIENT;
        ...
        if (dtls_srtp->role == DTLS_SRTP_ROLE_SERVER) {
            ...
            // PATCH: disable DTLS HelloVerifyRequest cookies. mbedTLS's
            // stateless cookie path can't reassemble fragmented ClientHellos
            // ("ClientHello fragmentation not supported" → FEATURE_UNAVAILABLE),
            // and Chrome's WebRTC ClientHello is 1413 bytes — always fragmented.
            // ICE already validates peer liveness before DTLS, so the cookie
            // round-trip is redundant for WebRTC.
...
void dtls_srtp_deinit(dtls_srtp_t *dtls_srtp)
{
    ...
    // PATCH: cookie context never initialized (cookies disabled to allow
    // fragmented ClientHello reassembly), so nothing to free here.
...
{
    // PATCH: same role override as dtls_srtp_init — libpeer would call
    // this with ROLE_SERVER and flip our forced CLIENT back. Pin to CLIENT.
    ...
    if (role != dtls_srtp->role) {
        // PATCH: cookies disabled — same rationale as init path.
